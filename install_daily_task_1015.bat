@echo off
REM Register Windows Task Scheduler: daily at 10:15, runs run_tests_scheduled.bat (no pause).
REM Run this BAT once as Administrator if "Access denied".

setlocal EnableExtensions
set "TASK_NAME=TestAutomation-Daily-1015"
...
echo Script: %PS1%
echo Schedule: every day at 10:15
echo.

REM Use PowerShell -File (avoids 0xC000013A when cmd closes while pytest/chromedriver still run).
REM If project path contains spaces, edit this line to: -File "\"%PS1%\""
schtasks /delete /tn "%TASK_NAME%" /f 2>nul
schtasks /create /tn "%TASK_NAME%" /tr "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File %PS1%" /sc daily /st 10:15 /f
if errorlevel 1 (
  echo.
