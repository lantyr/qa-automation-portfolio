...
class TestPCHomePage:
    ...
    def test_tc_pc_hp_001_home_open(self, driver):
        ...
        with allure.step("1. 前往首頁"):
            home.go_to_home()
            _screenshot(driver, "步驟1_首頁載入")

        with allure.step("2. 驗證 LOGO 可見"):
            home.wait_until_visible(HomePage.LOGO)
            _screenshot(driver, "步驟2_LOGO可見")

    # ────────────────────────────────────────────────────────────
    # TC-PC-HP-002
    # ────────────────────────────────────────────────────────────
    @allure.title("TC-PC-HP-002：大看板顯示正常")
    def test_tc_pc_hp_002_banner_display(self, driver):
    ...
    def test_tc_pc_hp_003_banner_link(self, driver):
        ...
        with allure.step("2-3. 切換至 iframe 驗證 banner 連結"):
            ...
            links = driver.find_elements(By.CSS_SELECTOR, "ul.bxslider li a[href]")
            assert len(links) > 0, "大看板內找不到任何含 href 的連結"
            href = links[0].get_attribute("href")
            assert href and href.strip(), "大看板第一個連結的 href 為空"
            _screenshot(driver, "步驟2_3_banner連結驗證")
            driver.switch_to.default_content()
    ...
    def test_tc_pc_hp_004_banner_switch(self, driver):
    ...
    def test_tc_pc_hp_005_news_display(self, driver):
        ...
        with allure.step("2. 驗證最新消息容器可見"):
            ...
            _screenshot(driver, "步驟2_消息容器可見")

        with allure.step("3. 驗證至少有一筆消息項目"):
            items = driver.find_elements(*HomePage.NEWS_LIST_ITEMS)
            assert len(items) > 0, "最新消息列表為空"
            _screenshot(driver, "步驟3_消息項目存在")

    # ────────────────────────────────────────────────────────────
    # TC-PC-HP-006
    # ────────────────────────────────────────────────────────────
    @allure.title("TC-PC-HP-006：最新消息連結正常")
    def test_tc_pc_hp_006_news_link(self, driver):
        ...
        with allure.step("2. 驗證第一筆消息連結有效"):
            ...
            _screenshot(driver, "步驟2_連結有效")

        with allure.step("3. 點擊連結"):
            original_handles = driver.window_handles
            link_el.click()
            _screenshot(driver, "步驟3_點擊後")
        ...
        with allure.step("4. 驗證新分頁開啟"):
            ...
            _screenshot(driver, "步驟4_新分頁URL驗證")
            driver.close()
    ...
    def test_tc_pc_hp_007_chatbot_display(self, driver):
        ...
        with allure.step("2. 驗證 Chatbot 按鈕可見"):
            ...
            _screenshot(driver, "步驟2_chatbot可見")

    # ────────────────────────────────────────────────────────────
    # TC-PC-HP-008
    # ────────────────────────────────────────────────────────────
    @allure.title("TC-PC-HP-008：右下角 Chatbot 浮水印連結正常")
    def test_tc_pc_hp_008_chatbot_link(self, driver):
